...
def layerVarianceFrame(enWeightedEtaPhi, etaPhi, frameNo, center, rng=None, maxE=1.0,
    ...
    left, width    = 0.1, 0.55 #|These determine where the subplots go
    ...
    axHist2D.text(1.05, 1.2, "Layer %s/30"%str(frameNo), transform=axHist2D.transAxes,fontsize=25)
...
def layerVarianceAnalysis(data, eventNo, title, mode=None, clusterID=0, rng=None, endLoop=0,
    ...
    if not cumulative:
        ...
        # Further data parsing
        xh,yh,zh,Eh = hits['x'], hits['y'], hits['z'], hit['en']                     #|Separating hits
    ...
    if os.path.exists(path):                                                                        #|Remove previous crap
        ...
        os.makedirs(path+"/frames")
    # Set up animation
    count    = 1                                                                                    #|Time step in ns
    minlayer = 1                                                                                    #|Minimum layer
    maxlayer = 30                                                                                   #|Maximum layer
    pbar     = progressbar("Rendering &count& frames:", maxlayer+1)
    pbar.start()
    # Render frames
    while count <= maxlayer:
        ...
        Eplt = np.extract(hits['layerID'] == count, Eh)
        filename = path + "/frames/"+str(count).zfill(3)+".gif"
        if len(etaplt) > 0:
...
def allLayerVarianceEvents(data):
    for event in data:
        for i in range(len(event[1])):
            cluster = event[1]
            hits = event[0]
            if cluster[i]['en'] > 300: #|Only keep energetic clusters
                pass
